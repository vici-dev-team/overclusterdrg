...
OUTPUT_FILE = "gonzalez_kcenter_results_compared.csv"
...
def gonzalez_deterministic_o_plus_1(X, k, z, seed=None):
    ...
    for _ in range(1, k):
        # Pick exactly the (z+1)-th farthest point
...
def gonzalez_randomized_pool(X, k, z, T=5, seed=None):
    ...
    for trial in range(T):
        ...
        for _ in range(1, k):
            # Extract the pool of the top (z+1) farthest points
            top_pool_indices = np.argpartition(min_dist, -(z + 1))[-(z + 1):]
            
            # Select uniformly at random from the pool
...
if __name__ == "__main__":
    ...
    # Initialize CSV file with wide formatting for easy comparison
    ...
    for size_req in target_sizes:
        ...
        for k in k_values:
            ...
            for p in outlier_percents:
                ...
                # We use the same seed to ensure all algorithms start from the exact same initial random center
                base_seed = 42
                ...
                # 3. Randomized Pool (Runs T times)
                t0 = time.time()
                ...
                print(f"    p={int(p*100)}% (z={z:4d}) | "
                      f"R_Std={r_std:.4f} ({t_std:.3f}s) | "
                      f"R_Det={r_det:.4f} ({t_det:.3f}s) | "
                      f"R_Rand={r_rand:.4f} ({t_rand:.3f}s)")
